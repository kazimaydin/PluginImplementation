...
namespace PluginImplementation.Utils.Cache
{
    public class MemoryCacheProvider : CacheProvider
    {
        ...
        public IEnumerable<IPlugin> GetOrCreateCacheForActivator<IPlugin>(IQueryable<IPlugin> query, string folder)
        {
            string key = "Activator.CreateInstance";

            CacheItem item = dictionary.GetOrAdd(
                key,
                (keyToFind) => {
                    return new CacheItem()
                    { Item = query.ToList(folder), AdditionTime = DateTime.Now };
                });

            foreach (var oneItem in ((List<IPlugin>)item.Item))
            {
                yield return oneItem;
            }
        }

        public bool RemoveFromCacheForActivator()
        {
            string key = "Activator.CreateInstance";
            CacheItem item = null;
            return dictionary.TryRemove(key, out item);
